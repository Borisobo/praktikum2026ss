...
class UI:
    ...
    def draw_cat_info(self, screen, cat, message=""):
        ...
        scroll_image = pygame.transform.scale(scroll_image, (900, 700))
        ...
        qualities_title = self.font.render("Eigenschaften:", True, (70, 45, 25))

        qualities_title_rect = qualities_title.get_rect(center=(center_x, scroll_rect.top + 330))

        screen.blit(qualities_title, qualities_title_rect)

        qualities_text = self.font.render(qualities, True, (70, 45, 25))

        qualities_rect = qualities_text.get_rect(center=(center_x, scroll_rect.top + 365))

        screen.blit(qualities_text, qualities_rect)

        take_text = self.font.render("E - Katze aufnehmen", True, (70, 120, 50))

        take_rect = take_text.get_rect(center=(center_x, scroll_rect.bottom - 200))

        screen.blit(take_text, take_rect)

        cancel_text = self.font.render("ESC - Schließen", True, (100, 70, 40))

        cancel_rect = cancel_text.get_rect(center=(center_x, scroll_rect.bottom - 230))
    ...
    def draw_level_complete(self, screen):
        scroll_image = pygame.image.load("assets/cat_info.PNG").convert_alpha()
        scroll_image = pygame.transform.scale(scroll_image, (850, 1100))
        scroll_rect = scroll_image.get_rect(center=(WIDTH // 2, HEIGHT // 2))
        screen.blit(scroll_image, scroll_rect)
        title = self.big_font.render("LEVEL GESCHAFFT", True, (70, 45, 25))
        ...
        enter_rect = enter.get_rect(center=(scroll_rect.centerx, scroll_rect.bottom - 200))
